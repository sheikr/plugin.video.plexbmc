...
class Plex:
    ...
    def check_signin_status(self, identifier):
        ...
        if temp_token:
            response = requests.get("%s/users/account?X-Plex-Token=%s" % (self.myplex_server, temp_token),
    ...
    def discover_all_servers(self):
            ...
            try:
                ...
                if gdm_client.discovery_complete and gdm_server_name:
                    ...
                    for device in gdm_server_name:
                        new_server = PlexMediaServer(name=device['serverName'], address=device['server'],
                                                     port=device['port'], discovery='discovery', uuid=device['uuid'])
        ...
        # Set to Disabled
            ...
            if settings.get_setting('ipaddress'):
                ...
                printDebug.info("PleXBMC -> Settings hostname and port: %s : %s" % (
                settings.get_setting('ipaddress'), settings.get_setting('port')))

                local_server = PlexMediaServer(address=settings.get_setting('ipaddress'),
                                               port=settings.get_setting('port'), discovery='local')
    ...
    def talk_to_myplex(self, path, renew=False, type='get'):
        ...
        try:
            if type == 'get':
                response = requests.get("%s%s" % (self.myplex_server, path), params=self.plex_identification(),
                                        verify=True, timeout=(3, 10))
            elif type == 'get2':
                response = requests.get("%s%s" % (self.myplex_server, path), headers=self.plex_identification(),
                                        verify=True, timeout=(3, 10))
            elif type == 'post':
                response = requests.post("%s%s" % (self.myplex_server, path), data='',
                                         headers=self.plex_identification(), verify=True, timeout=(3, 10))
    ...
    def get_server_from_ip(self, ip):
        ...
        for server in self.server_list.values():
            ...
            printDebug.debug(
                "[%s] - checking ip:%s against server ip %s" % (server.get_name(), ip, server.get_address()))
