...
class CacheControl:
    ...
    def deleteCache(self, force=False):
        ...
        for item in file_list:
            ...
            if force and persistent_cache_suffix in item:
                printDebug("Force deletion of persistent cache file")
            elif cache_suffix not in item:
                continue

            if xbmcvfs.delete(self.cache_location + item):
                printDebug.debug("SUCCESSFUL: removed %s" % item)
            else:
                printDebug.debug("UNSUCESSFUL: did not remove %s" % item)
